...
function showError(request,status,error){
    ...
    if(request.status==403&&reUrl!="http://localhost:8080/api/email"){
        ...
        loginBool=false;
        return;
    }else if(reUrl=="http://localhost:8080/api/cancleArticle"){
        return;
    }else{
        alert("통신 실패");
    }
...
function getNewAccesToken2(requestUrl,data){
    console.log('새토큰 필요');
    $.ajax({
        type: 'POST',
        url: requestUrl,
...
function fisrtRequest2(requestUrl,data){
    reUrl=requestUrl;
    $.ajax({
        type: 'POST',
        url: requestUrl,
...
function cancleArticle() {
    ...
    let data=JSON.stringify({
		 "text":$('#'+textAreaId).summernote('code')
	});
    result=fisrtRequest2(requestUrl,data);
}
